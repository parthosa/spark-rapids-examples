...
class GitHubNotebookFetcher:
    ...
    def fetch_notebook_content(repo_url: str, notebook_path: str, branch: str = "main") -> str:
        ...
        Args:
            ...
            branch: Branch name to fetch from, defaults to 'main'

        Returns:
            The notebook content as a string

        Raises:
            Exception: If the notebook cannot be fetched, with details about the failure
        """
        raw_url = GitHubNotebookFetcher.get_raw_notebook_url(repo_url, notebook_path, branch)
        response = requests.get(raw_url)
        if response.status_code != 200:
            raise Exception(
                f"Failed to fetch notebook from GitHub.\n"
                f"URL: {raw_url}\n"
